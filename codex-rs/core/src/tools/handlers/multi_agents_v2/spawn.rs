...
use crate::agent::child_config::SpawnModelRoute;
...
> {
    ...
    let prepared = prepare_agent_spawn_config(
        ...
        SpawnConfigOptions {
            ...
            full_history_fork: requested_full_history_fork,
            role_name,
            model: args.model.as_deref(),
            reasoning_effort: args.reasoning_effort.clone(),
            model_route: if is_external_spawn {
                SpawnModelRoute::ExternalProvider
            } else {
    ...
    .await;
    let role_tag = prepared.role_name.as_deref().unwrap_or(DEFAULT_ROLE_NAME);
